...
class DateTimeTests {
	...
	void testNextFriday13Adjuster() {
		assertEquals(LocalDate.parse("2022-05-13"), LocalDate.parse("2021-12-22")
				.with(new NextFriday13Adjuster()));
	}
	...
	void testGetAge() {
		...
		assertEquals(2021 - 1799, getAgeDate(birthAS, ld1));
		assertEquals(2020 - 1799, getAgeDate(birthAS, ld2));
		assertEquals(2020 - 1799, getAgeDate(birthAS, ld3));
	}
	...
	void instantTest() {
		...
		zdt = zdt.withZoneSameInstant(ZoneId.of("GMT+8"));
		System.out.printf("instant view %s, LocalDateTime %s, zonedDateTime %s\n", instant, ltd, zdt);
	}
	...
	void zoneIDsTest() {
		...
		for(String zone: ZoneId.getAvailableZoneIds()) {
			if (zone.toLowerCase().contains("toronto")) {
				System.out.println(zdt.withZoneSameInstant(ZoneId.of(zone)));
			}
		}
	}
}
